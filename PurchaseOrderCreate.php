<?php
  ...
  $item_id = $_REQUEST['fPurchaseOrderId'];
  $avendorList = $oAssetVendor->getAllVendorInfo();
  if(isset($_REQUEST['fPurchaseOrderId']) && $aRequest['action'] != 'edit')
  {
 ...
 if($aRequest['action'] == 'edit')
  {
	$item_id = $aRequest['fPurchaseOrderId'];
         ...
         <div class="container-fluid">
            ...
            <!-- END PAGE HEADER-->
               ...
               <div class="span12">
                               ...
                               <div class="controls">
                                       ...
                                       <select class="span8 chosen"   tabindex="4"  data-placeholder="Choose a Unit Id" name="fUnitId" id="fUnitId" onChange="getDivision(this.value);">
											...
											<?php
                                             ...
                                             <option value="<?php echo $aUnit['id_unit']; ?>" <?php if((!empty($apurchaserequest_info['id_unit'])? $apurchaserequest_info['id_unit']:$aRequest['fUnitId']) == $aUnit['id_unit'] || $aEditPurchaseOrder['id_unit'] == $aUnit['id_unit']) { echo 'selected=selected' ;}?>><?php echo $aUnit['unit_name']; ?></option>
                                             <?php
											  }
											 ?>
                                          </select>
                               ...
                               <div class="controls" >
                                       ...
                                       <select class="span8 fDivisionId" data-placeholder="Choose a Division"  name="fDepartmentId" id="unitwisedivisionList"> 
											 ...
											 <?php
											  ...
											  {
											 ?>
                                             <option value="<?php echo $aDivision['id_division']; ?>" <?php if((!empty($apurchaserequest_info['id_department'])? $apurchaserequest_info['id_department']:$aRequest['fDepartmentId']) == $aDivision['id_division'] || (!empty($aEditPurchaseOrder['id_department'])? $aEditPurchaseOrder['id_department']:$aRequest['fDepartmentId']) == $aDivision['id_division']) { echo 'selected=selected' ;}?>><?php echo $aDivision['division_name']; ?></option>
                                             <?php
											  }
											 ?>
                                          </select>
									  ...
									  <div class="control-group">
                                             ...
                                             <div class="controls">
                                               <span class="text"><b> <?php echo strtoupper($apurchaserequest_info['quote_approval']['vendor_name']); ?></b></span> <input type="hidden" name="fvendorId" value="<?php echo (!empty($apurchaserequest_info['quote_approval']['id_vendor'])? $apurchaserequest_info['quote_approval']['id_vendor']:$aRequest['fvendorId']);?>"/>
                                   ...
                                   <?php } ?>
									...
									<tbody>
                                        ...
                                        <td>
                                          ...
                                          <select class="m-wrap margin group1" name="fGroup1[]" id="fGroupItem<?php echo $i;?>" onChange="getGroup2ItemListing(this.value,this.id);">
                                            <option value="0" selected="selected" >Choose the ItemGroup 1 </option>
                                             ...
                                             <option value="<?php echo $aItemGroup1['id_itemgroup1']; ?>" <?php if((!empty($purchaseitem['id_itemgroup1'])? $purchaseitem['id_itemgroup1']:$aRequest['fGroup1']) == $aItemGroup1['id_itemgroup1']) { echo 'selected=selected' ;}?>><?php echo $aItemGroup1['itemgroup1_name']; ?></option>
                                             <?php
											  }
											 ?>
                                          </select>
                                        
                                          <select class="m-wrap group2" name="fGroup2[]" id="Group2ItemList<?php echo $i;?>" onChange="getItemListing(this.value,this.id);">
                                             ...
                                             <option value="<?php echo $aItemGroup2['id_itemgroup2']; ?>" <?php if((!empty($purchaseitem['id_itemgroup2'])? $purchaseitem['id_itemgroup2']:$aRequest['fGroup2']) == $aItemGroup2['id_itemgroup2']) { echo 'selected=selected' ;}?>><?php echo $aItemGroup2['itemgroup2_name']; ?></option>
                                             <?php
											  }
											 ?>
                                          </select>
                                          
                                           <select class="m-wrap  nextRow margin items" name="fItemName[]" id="ItemList<?php echo $i;?>">
                                    <option value="0" >Choose the Item</option>
											 <?php
											  ...
											  {
											 ?>
                                             <option value="<?php echo $aItem['id_item']; ?>" <?php if((!empty($purchaseitem['id_item'])? $purchaseitem['id_item']:$aRequest['fItemName']) == $aItem['id_item']) { echo 'selected=selected' ;}?>><?php echo $aItem['item_name']; ?></option>
                                             <?php
											  }
											 ?>
                                          </select>
                   ...
                   <td><input type="text" class="m-wrap xsmall" name="fQuantity[]" value="<?php echo (!empty($purchaseitem['balanced_qty'])? $purchaseitem['balanced_qty']:$aRequest['fQuantity']);?>" onKeyUp="qtytotalpricecalc(this.id);  checkQty(this.id,this.value,'<?php echo $aRequest['id'];?>','<?php echo $purchaseitem['id_pr_item'];?>');"id="qty<?php echo $i;?>" style="margin-bottom: 5px;"/>
						  ...
						  <!--Add the PO END-->  
						   <!--Edit the PO END-->  
                           <?php }
						     else if(isset($_GET['fPurchaseOrderId']) && $aRequest['action'] == 'edit'){?>
                   ...
                   <td><input type="text" class="m-wrap xsmall" name="fQuantity[]" value="<?php echo $purchaseitem['balanced_qty'];?>" onKeyUp="qtytotalpricecalc(this.id);  checkQty(this.id,this.value,'<?php echo $aRequest['id'];?>','<?php echo $purchaseitem['id_pr_item'];?>');"id="qty<?php echo $i;?>" style="margin-bottom: 5px;"/>
                                    ...
                                    <option value="0">Choose the Manufacturer</option>
                                         ...
                                         <br>  <br>
                                         <div class="input-append date date-picker" data-date="<?php echo date('d-m-Y');?>" data-date-format="dd-mm-yyyy">
													<input class="m-wrap m-ctrl-small date-picker span8" placeholder="Required Date" size="10" type="text" name="fRequireDate[]" value="<?php echo date("d-m-Y",strtotime( $purchaseitem['due_date']));?>" style="width:100px;" ><span class="add-on"><i class="icon-calendar"></i></span>
												 </div>
										  ...
										  <?php 
											  ...
											  /* $xid = fGroupItem.$i;
										   $xid1 = Group2ItemList.$i;
										  $xfooter .=" <script type='text/javascript'>jQuery(document).ready(function() { 
							$(function () {
							getVendorItemGroup1('".$aEditPurchaseOrder['id_vendor']."','".$purchaseitem['id_itemgroup1']."');
							getGroup2ItemListing('".$purchaseitem['id_itemgroup1']."','".$xid."','".$purchaseitem['id_itemgroup2']."');
							getItemListing('".$purchaseitem['id_itemgroup2']."','".$xid1."','".$purchaseitem['id_item']."');
														});
							});
							</script>";*/
                   ...
                   <td><input type="text" class="m-wrap xsmall" name="fQuantity[]" value="<?php echo $purchaseitem['qty'];?>" onKeyUp="qtytotalpricecalc(this.id);"id="qty<?php echo $j;?>" style="margin-bottom: 5px;"/>
                          ...
                          <?php } ?>
                              ...
                              <?php if($aRequest['action'] != 'edit') 
                              {
                               $req_date = $edit_result['due_date'];}
                           else{  
                           $req_date= $aEditPurchaseOrder['po_duedate']; }
                              ?>
								 ...
								 {
									 echo date('d-m-Y',strtotime( $req_date ));
								 }?>" />
                              </div>
...
if($aRequest['action'] == 'edit' && $aRequest['submits']=='approval') {
    ...
    <div class="control-group">
						...
						{
						?>
							<input type="hidden" name="fPurchaseOrderId" value="<?php echo $aRequest['fPurchaseOrderId'];?>"/>
                            <input type="hidden" name="fApproval" value="<?php echo $aRequest['submits'];?>"/>
							 <input type="hidden" name="type" value="<?php echo $aRequest['fOrderType'];?>"/>
   ...
   <!-- Load javascripts at bottom, this will reduce page load time -->
    ...
    <script type="text/javascript">
			...
			var dropresult = addParam(url, "fPurchaseOrderId",id);	
 ...
 <script type="text/javascript">
 		...
 		function getGroup2ItemListing(value,id,group2)
		 {
			
			var dataStr = 'action=getGroup2ItemList&Group1Id='+value+'&group2Id='+group2;
		 ...
		 function getItemListing(value,id,itemid)
		 {
			var dataStr = 'action=getItemLists&Group2Id='+value+'&id='+itemid;
			 $.ajax({
			   type: 'POST',
			   url: 'ajax/ajax.php',
			   data: dataStr,
	  ...
	  function getDivision(value,divisionid)
	  {
	   ...
	   function getVendorItemGroup1(value,id)
		 {
